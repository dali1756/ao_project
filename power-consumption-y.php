<?php 
	...
	if($_GET['search']){
		...
		if(isset($_GET['m_year_st'])   && isset($_GET['m_month_st']) )
		{
			...
			":year_end_220"=> $year_st_220 + 1 ,
			":month_end1_220"=> $month_st_220."-01 00:00:59",
			":dong" => $dong
			); 
			...
			if(count($room_data) > 0 )
			{
				...
				{ 
					...
					$room_total_ele_110 = $room_total_ele - $room_total_ele_220;
...
<script>
	...
	{	
		...
		location.replace("model/power-consumption-y_report.php?dong="+dong+"&st="+year_st+"&ed="+month_st+"&dong_name="+dong_name+"&total="+total+"&total_1="+total_1+"&total_2="+total_2);
	}
</script>
