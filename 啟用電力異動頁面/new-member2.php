	...
	<!-- 住宿中心圖示智慧操作系統 -->
		<div class="inner6">       
			<div class="row justify-content-center">
				<div id="content" class='col-auto p-4 text-center'> 
					<input type='hidden' value='<?php echo $user_sn ?>'>
				</div>
			</div>
		</div>
	<!-- End 住宿中心圖示智慧操作系統 -->
...
<script>
	...
	function iconPage(status,img_url,click_function,text){
		var page;
		if(status){
			page = `<a href='#!'>
						<img class='mb-3' src='img/${img_url}' alt='${text}' onclick='${click_function}'>
						<h4>${text}</h4> 
					</a>`;
		}else{
			page =  `<p>${text}</<p>`;
	...
	function Ctrl_power(type,dataType){
		let id = $('#content input').val();
		console.log('id',id);
		$.ajax({
                // url:'./API/APP/Get_CanRoomOfMemberUse.php?member_id='+id,
                url:'../ndhu_new/API/APP/Get_CanRoomOfMemberUse.php?member_id='+id,
                type:type,
                dataType:dataType,
				async:false,
                success:function(data){
					...
					let info =  data.info;
					$.each(info,(i,v)=>{
						let member_can_use_room = v.member_can_use_room;
						let powerstaus = v.powerstaus;
						let room_strings = v.room_strings;
						let balance = v.balance;

						if(room_strings == ''){
							iconPage(1,'36非住宿生無法啟用電力.png','','未入住無法啟用電力');
						}else{
							if(balance > 0 && member_can_use_room) {
								// console.log('Y');
								// 當powerstaus=1開電，顯示不啟用電力icon，0=關電，顯示啟用電力icon
								powerstaus ?
								iconPage(1,'39.不啟用電力.png',`setElectricCtrl(${id})`,'不啟用電力') : iconPage(1,'37啟用電力.png',`setElectricCtrl(${id})`,'啟用電力'); 
							}else if(returnCode == 1){
								iconPage(1,'40.無開關電權限.png','',returnMessage);
							}else if(balance <= 0){
								iconPage(1,'38餘額不足無法啟用電力.png',`NosetElectricCtrl(${id})`,'餘額不足無法啟用電力'); 
							}else{
								iconPage(1,'40.無開關電權限.png','',`注意：${returnCode} ${returnMessage}`); 
							}
						}
