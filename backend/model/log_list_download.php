<?php
	...
	$sql = "SELECT * FROM `log_list` ORDER BY id DESC";
	...
	if($data) {
		...
		foreach($data as $k => $row) {
			...
			$xls->setCellValueByColumnAndRow($i++, $k+3, $row['content']);
			$xls->setCellValueByColumnAndRow($i++, $k+3, $data_type[$row['data_type']]);
		}
