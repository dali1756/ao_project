...
{  
    ...
    $year_st = trim($_GET['st']);
    $month_st = trim($_GET['ed']);
    $search   = trim($_GET['search']);
    $year_end = trim($_GET['st']);
    # 結束月取隔月一號的第一筆資料
    ...
    if ($dong && $year_st && $month_st && $year_end) {
        $filename  = "棟別年用電總計.csv";
        ...
        foreach ($room_data as $k => $row) { 
            ...
            $room_ele = $row['use_amonut']; // 單間房的110V總度數
            $room_ele_220 = $row['use_amonut_220']; // 單間房的220V總度數
            ...
            if ($k == 0) 
            {
                ...
                //$xls->setCellValueByColumnAndRow(1, $k + 2, "總計用電：");
                //$xls->setCellValueByColumnAndRow(2, $k + 2, $total);
                    $xls->setCellValueByColumnAndRow(1, $k + 1, $body_date);
                    $xls->setCellValueByColumnAndRow(1, $k + 2, $dong_name);  
                    $xls->setCellValueByColumnAndRow(2, $k + 2, "棟別年用電總計");                 
        ...
        die(header("location: ../power-consumption-y.php?error=6"));
    }
