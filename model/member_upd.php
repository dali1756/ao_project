...
if(in_array('1', $member_grp) & !in_array('1', $mem_grp_old)) { // 管理員群組上限
...
$sql = "SELECT * FROM `member` WHERE `username` = '{$username}' AND id != '{$id}'";
$tmp = $PDOLink->prepare($sql);
$tmp->execute();
$rs  = $tmp->fetch();
...
if($id_card != '' && $id_card != str_pad('0',10,'0',STR_PAD_LEFT) && $identity == '0') {  // 重複卡號
	$sql = "SELECT * FROM `member` WHERE `id_card` = '{$id_card}' AND id != '{$id}'";
	$tmp = $PDOLink->prepare($sql);
	$tmp->execute();
	$rs  = $tmp->fetch();
	$check_card = $rs['id'];
...
if($flag !== false) {
	...
	// 編號、房號、卡號、登入身分變更-- 20200813
	if($room_num != $room_num_old || $id_card != $id_card_old || $identity != $identity_old || $username != $username_old) {
	...
	// if($member_grp !== $mem_old_arr) {
		// $hw_cmd = array('op' => 'MemberGroup_Initialize', 'table' => 'member', 'id' => $id, 'group_id_old' => $mem_old_arr);
		// insert_system_setting($hw_cmd);		
	// }
	
	// log_list
	$content = "更新: [member] id:{$id}, 管理員:{$admin_id}/{$admin}";
	...
	header("Location: ../member_edit.php?id={$id}&error=1");
